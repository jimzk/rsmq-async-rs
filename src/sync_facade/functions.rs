...
#[derive(Clone)]
pub struct RsmqFunctionsSync<T: ConnectionLike> {
    pub(crate) ns: String,
    pub(crate) realtime: bool,
    pub(crate) conn: std::marker::PhantomData<T>,
}

impl<T: ConnectionLike> std::fmt::Debug for RsmqFunctionsSync<T> {
...
#[derive(Debug, Clone)]
pub struct CachedScriptSync {
    change_message_visibility_sha1: String,
    receive_message_sha1: String,
}

impl CachedScriptSync {
    fn init<T: ConnectionLike>(conn: &mut T) -> RsmqResult<Self> {
    ...
    fn invoke_change_message_visibility<R, T: ConnectionLike>(
        &self,
        conn: &mut T,
        key1: String,
        key2: String,
    ...
    fn invoke_receive_message<R, T: ConnectionLike>(
        &self,
        conn: &mut T,
        key1: String,
        key2: String,
...
impl<T: ConnectionLike> RsmqFunctionsSync<T> {
    ...
    pub fn change_message_visibility(
        ...
        message_id: &str,
        hidden: Duration,
        cached_script: &CachedScriptSync,
    ) -> RsmqResult<()> {
        ...
        cached_script.invoke_change_message_visibility::<(), T>(
            conn,
    ...
    pub fn load_scripts(&self, conn: &mut T) -> RsmqResult<CachedScriptSync> {
        CachedScriptSync::init(conn)
    }
    ...
    pub fn pop_message<E: TryFrom<RedisBytes, Error = Vec<u8>>>(
        ...
        let result: (bool, String, Vec<u8>, u64, u64) = cached_script.invoke_receive_message(
            conn,
            format!("{}:{}", self.ns, qname),
            queue.ts.to_string(),
            queue.ts.to_string(),
    ...
    fn get_queue(&self, conn: &mut T, qname: &str, uid: bool) -> RsmqResult<QueueDescriptor> {
        ...
        let quid = if uid {
            Some(radix_36(time).to_string() + &RsmqFunctionsSync::<T>::make_id(22)?)
        } else {
            None
        };
